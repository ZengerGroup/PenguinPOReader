    ...
    internal class WorkbookHandler
    {
        ...
        public void Update(PdfReader reader)
        {
            try
            {
                ...
                var updateRequest = Sheets.Spreadsheets.Values.Update(valueRange, Configurator.SheetId, String.Format("{0}!A{1}:M{1}", sheetName, row));
        ...
        private List<object> GetRowData(PdfReader reader)
        {
            ...
            {
                DateTime.Now.ToString("MM/dd"), reader.PO, "", reader.Quantity, reader.ISBN, reader.Title, reader.Color, reader.Stock, reader.UV, reader.LAM, 
